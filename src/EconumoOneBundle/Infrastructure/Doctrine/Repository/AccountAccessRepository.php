...
class AccountAccessRepository extends ServiceEntityRepository implements AccountAccessRepositoryInterface
{
    ...
    {
        $dql = <<<'DQL'
SELECT a.id FROM App\EconumoOneBundle\Domain\Entity\AccountAccess aa
JOIN App\EconumoOneBundle\Domain\Entity\Account a WITH a = aa.account AND aa.user = :user
GROUP BY a.id
DQL;
    ...
    public function getIssuedAccess(Id $userId): array
    {
        $dql = <<<'DQL'
SELECT a.id FROM App\EconumoOneBundle\Domain\Entity\AccountAccess aa
JOIN App\EconumoOneBundle\Domain\Entity\Account a WITH a = aa.account AND a.user = :user
GROUP BY a.id
DQL;
        $query = $this->getEntityManager()->createQuery($dql)
            ->setParameter('user', $this->getEntityManager()->getReference(User::class, $userId));
